...
class BaseModel(txdbapi.DatabaseModel):
    db = txdbapi.ConnectionPool("MySQLdb", db="test")

    @classmethod
    @defer.inlineCallbacks
    def setup(cls):
        yield BaseModel.db.runOperation("drop table if exists asd")
        yield BaseModel.db.runOperation(
            "create table asd "
            "(id integer auto_increment primary key, age int, name text)")
